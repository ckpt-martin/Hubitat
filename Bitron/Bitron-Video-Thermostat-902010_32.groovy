...
metadata {
	definition (name: "Bitron Video Thermostat 902010/32", namespace: "ckpt-martin", author: "ckpt-martin") {
		...
		fingerprint profileId: "0104", endpointId: "01", inClusters: "0000,0001,0003,000A,0020,0201,0204,0B05", outClusters: "0003,0019", manufacturer: "Bitron Home", model: "902010/32"
	}
...
//		input("battalarm", "enum", title: "Enable Battery Alarm?", options: ["No", "Yes"], defaultValue: "No", required: false, displayDuringSetup: false)
...
def parse(String description) {
	...
	if (description?.startsWith("read attr -")) {
		...
		if (descMap.cluster == "0201" && descMap.attrId == "0000")
		{
            ...
            correctedValue = descMap.value.substring(2, 4) + descMap.value.substring(0, 2)
			map.value = getTemperature(correctedValue)
		...
		{
			log.debug "BATTERY VOLTAGE (uint8): $descMap.value"
			map.name = "batteryVoltage"
			def batteryVoltageRaw = getBatteryVoltage(descMap.value)
            batteryVoltageRaw = batteryVoltageRaw/10
			log.debug "BATTERY VOLTAGE: $batteryVoltageRaw V"
			map.value = "$batteryVoltageRaw V"
            map2 = [
                name: "battery",
                value: 100
            ]
			if (batteryVoltageRaw <= 2.5)
			{
				map2.value = 17
                log.debug "BATTERY LEVEL: $map2.value %"
			}
			else if (batteryVoltageRaw == 2.6)
			{
				map2.value = 33
                log.debug "BATTERY LEVEL: $map2.value %"
		    }
				else if (batteryVoltageRaw == 2.7)
			{
				map2.value = 50
                log.debug "BATTERY LEVEL: $map2.value %"
		    }
			else if (batteryVoltageRaw == 2.8)
			{
				map2.value = 67
                log.debug "BATTERY LEVEL: $map2.value %"
			}
			else if (batteryVoltageRaw == 2.9)
			{
				map2.value = 83
                log.debug "BATTERY LEVEL: $map2.value %"
			}
			else if (batteryVoltageRaw >= 3.0)
			{
				map2.value = 100
                log.debug "BATTERY LEVEL: $map2.value %"
			}
		}
		else if (descMap.cluster == "0001" && descMap.attrId == "0032")
		{
			log.debug "BATTERY AHr Rating (uint16): $descMap.value"
			map.name = "batteryAHr"
			def batteryAHrRating = intToDec(descMap.value)
            batteryAHrRating = batteryAHrRating*10
			log.debug "BATTERY AHr Rating: $batteryAHrRating mAh"
			map.value = "$batteryAHrRating mAh"
		}
		else if (descMap.cluster == "0001" && descMap.attrId == "0033")
		{
			log.debug "BATTERY Quantity (uint8): $descMap.value"
			map.name = "batteryQuantity"
			def batteryNumber = intToDec(descMap.value)
			log.debug "BATTERY Quantity: $batteryNumber"
			map.value = batteryNumber
		}
		else if (descMap.cluster == "0001" && descMap.attrId == "0034")
		{
			log.debug "BATTERY Rated Voltage (uint8): $descMap.value"
			map.name = "batteryRatedVoltage"
			def batteryVRating = intToDec(descMap.value)
            batteryVRating = batteryVRating/10
			log.debug "BATTERY Rated Voltage: $batteryVRating V"
			map.value = "$batteryVRating V"
		}
		else if (descMap.cluster == "0001" && descMap.attrId == "0036")
		{
			log.debug "BATTERY Voltage Min (uint8): $descMap.value"
			map.name = "batteryVoltageMin"
			def batteryVMin = getBatteryVoltage(descMap.value)
            batteryVMin = batteryVMin/10
			log.debug "BATTERY Voltage Min: $batteryVMin V"
			map.value = "$batteryVMin V"
		}
		else if (descMap.cluster == "0001" && descMap.attrId == "0035")
		{
			log.debug "BATTERY ALARM MASK (map8): $descMap.value"
			map.name = "batteryAlarmMask"
//			def batteryAMask = getBatteryVoltage(descMap.value)
//          batteryVMin = batteryVMin/10
//			log.debug "BATTERY Voltage Min: $batteryVMin V"
		...
		{
			log.debug "BATTERY ALARM STATE (map32): $descMap.value"
			map.name = "batteryAState"
//			def batteryVMin = getBatteryVoltage(descMap.value)
//          batteryVMin = batteryVMin/10
//			log.debug "BATTERY Voltage Min: $batteryVMin V"
			map.value = descMap.value
	...
	if (map2) {
        ...
        log.debug "Parse map2 returned $map2"
	}
...
def getTemperature(value) {
	if (value != null) {
    	...
    	log.debug("Readable TEMP $celsius")
...
def configure() {
	def cmds =
		...
		zigbee.configureReporting(0x0201, 0x0008, 0x20, 10, 300, 0x05) +   //Attribute ID 0x0008 = pi heating demand, Data Type: U8BIT
		zigbee.configureReporting(0x0201, 0x0000, 0x29, 10, 300, 50) + 	//Attribute ID 0x0000 = local temperature, Data Type: S16BIT
		...
		zigbee.configureReporting(0x0001, 0x0020, 0x20, 10, 7200, 0x01)	//Attribute ID 0x0020 = battery voltage, Data Type: U8BIT
//		zigbee.configureReporting(0x0001, 0x003C, 0x1B, 600, 21600, 0x01) 	//Attribute ID 0x003C = battery alarm state, Data Type: map32
...
def refresh() {
	def cmds =
		...
		//zigbee.readAttribute(0x001, 0x0021) +	//Read BatteryPercentage
		...
		zigbee.readAttribute(0x001, 0x003E) +	//Read BatteryAlarmState
...
def updated() {
	...
	log.info "battalarm : $settings.battalarm"
	if (settings.battalarm == "Yes")
	{
		log.info "Battery Alarm enabled"
		battallarm = 0x00
	}
	else
	{
		log.info "No Battery Alarm enabled"
		battalarm = 0x01
		settings.battalarm = "No"
	}
...
//    }
	def cmds = 
		...
		zigbee.writeAttribute(0x001, 0x0035, 0x18, battalarm) +
		zigbee.readAttribute(0x001, 0x0035) +
